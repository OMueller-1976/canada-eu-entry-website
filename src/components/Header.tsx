    ...
    <header className="sticky top-0 z-40 border-b border-navy/10 bg-offwhite/95 backdrop-blur">
      <Container className="flex h-20 items-center justify-between gap-4">
        <Link href="/" className="flex shrink-0 flex-col leading-tight">
          <span className="font-serif text-xl font-bold uppercase tracking-tight text-navy">
            {SITE.brand}
          </span>
          <span className="text-xs font-medium text-accent">{SITE.byline}</span>
        </Link>

        <nav aria-label="Primary" className="hidden items-center gap-4 xl:gap-5 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="whitespace-nowrap text-[13px] font-medium text-graphite transition-colors hover:text-navy"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden shrink-0 items-center gap-3 lg:flex">
          <Link
            ...
            className="whitespace-nowrap text-[13px] font-medium text-navy underline-offset-4 hover:underline"
          >
            {SECONDARY_CTA.label}
          </Link>
          <Link
            href={PRIMARY_CTA.href}
            className="inline-flex items-center justify-center whitespace-nowrap rounded-sm bg-accent px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-accent-700"
          >
            {PRIMARY_CTA.label}
          </Link>
        </div>
